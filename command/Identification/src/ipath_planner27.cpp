...
ros::Subscriber feedback0;
ros::Subscriber feedback1;
ros::Subscriber feedback2;
ros::Subscriber feedback3;
ros::Publisher iwaypointPublisher2;
ros::Publisher iwaypointPublisher3;
ros::Publisher iwaypointPublisher4;
ros::Publisher iwaypointPublisher5;
...
TASK_T t;
UAV u;
int flag2 = 0;//Mark if the target assignment has been executed
int flag = 0;
...
void task_assignment()
{
	...
	while(um.get_uav(1)){
		...
		u = um.get_u();
		task_result[0].Uid = u.id;
		um.update_states(u.id);
		break;
	}
	//For UAV2
	dis[1][r1] = 9999999;
	...
	while(um.get_uav(1)){
		...
		u = um.get_u();
		task_result[2].Uid = u.id;
		um.update_states(u.id);
		break;
	}
}
void deal2(const commond_msgs::Waypoint::ConstPtr &msg)
{
	commond_msgs::Waypoint feedback;
	commond_msgs::Waypoint inew_waypoint;
	TASK_T temp;
	ros::NodeHandle nh2;
	feedback = *msg;
	iwaypointPublisher5 = nh2.advertise<commond_msgs::Waypoint>("/bebop_5/waypoint_path", 10);
	if(feedback.result == 0)
	{
    	while(um.get_uav(1))
    	{
    		u = um.get_u();
    		um.update_states(u.id);
    		temp = tm.find_task(feedback.tid);
    		temp.Uid = u.id;
			inew_waypoint.w[0] = temp.t_des[0];
			inew_waypoint.w[1] = temp.t_des[1];
			inew_waypoint.w[2] = temp.t_des[2];
			inew_waypoint.lat = 0;
			inew_waypoint.lon = 0;
			...
			inew_waypoint.uid = temp.Uid;
			inew_waypoint.tid = temp.id;
			inew_waypoint.result = -1;
			inew_waypoint.t_state = 1;
			iwaypointPublisher5.publish(inew_waypoint);
			break;
    	}
	}
}
void deal3(const commond_msgs::Waypoint::ConstPtr &msg)
{
	commond_msgs::Waypoint feedback;
	commond_msgs::Waypoint inew_waypoint;
	TASK_T temp;
	ros::NodeHandle nh3;
	feedback = *msg;
	iwaypointPublisher5 = nh3.advertise<commond_msgs::Waypoint>("/bebop_5/waypoint_path", 10);
	if(feedback.result == 0)
	{
    	while(um.get_uav(1))
    	{
...
void deal4(const commond_msgs::Waypoint::ConstPtr &msg)
{
	commond_msgs::Waypoint feedback;
	commond_msgs::Waypoint inew_waypoint;
	ros::NodeHandle nh4;
	TASK_T temp;
	feedback = *msg;
	iwaypointPublisher5 = nh4.advertise<commond_msgs::Waypoint>("/bebop_5/waypoint_path", 10);
	if(feedback.result == 0)
	{
    	while(um.get_uav(1))
    	{
...
void deal5(const commond_msgs::Waypoint::ConstPtr &msg)
{
	commond_msgs::Waypoint feedback;
	commond_msgs::Waypoint inew_waypoint;
	ros::NodeHandle nh5;
	TASK_T temp;
	feedback = *msg;
	iwaypointPublisher5 = nh5.advertise<commond_msgs::Waypoint>("/bebop_5/waypoint_path", 10);
	if(feedback.result == 0)
	{
    	while(um.get_uav(1))
    	{
...
void iwaypoint_path_Callback(const nav_msgs::Odometry::ConstPtr &msg)
{
	...
	//iwaypointPublisher5 = nh.advertise<commond_msgs::Waypoint>("/bebop_5/waypoint_path", 10);
    commond_msgs::Waypoint inew_waypoint;
    nav_msgs::Odometry bebop_state;
    bebop_state = *msg;
    ...
    if(flag == 1 && flag2 == 0)
    {
 		...
 		for(j=0;j<3;j++)
 		{
 			inew_waypoint.w[0] = task_result[j].t_des[0];
   			...
   			inew_waypoint.takeoff =false;
   			inew_waypoint.clear_wp_list = false;
   			inew_waypoint.uid = task_result[j].Uid;
   			inew_waypoint.tid = task_result[j].id;
   			inew_waypoint.result = -1;
   			inew_waypoint.t_state = 1;
   			switch(task_result[j].Uid){
   				case 0:
...
int main(int argc, char **argv)
{
	...
	feedback0 = nh_.subscribe("/bebop_2/feedback",10,deal2);
	feedback1 = nh_.subscribe("/bebop_3/feedback",10,deal3);
	feedback2 = nh_.subscribe("/bebop_4/feedback",10,deal4);
	feedback3 = nh_.subscribe("/bebop_5/feedback",10,deal5);
	ros::spin();
	return 0;
}
